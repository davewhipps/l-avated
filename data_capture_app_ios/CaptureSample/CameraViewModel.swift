...
class CameraViewModel: NSObject, ObservableObject, AVSpeechSynthesizerDelegate {
    ...
    /// This enum includes a value for each terrain class supported by the model that we are gathering data for
    ///
    enum TerrainType: String, CaseIterable, Identifiable, CustomStringConvertible {
        case NONE_SELECTED,
        bankedLeft,
        bankedRight,
        flatEven,
        ...
        var description : String {
            switch self {
                case .NONE_SELECTED: return "-NONE-"
                case .bankedLeft: return "Banked Left"
                case .bankedRight: return "Banked Right"
                case .flatEven: return "Flat-Even"
                case .flatIrregular: return "Irregular"
                case .grass: return "Grass"
                case .slopedDown: return "Sloped Down"
                case .slopedUp: return "Sloped Up"
    ...
    /// This property returns the current terrainType. Defaults to NONE_SELECTED so that the user must set it
    @Published var terrainType: TerrainType = .flatEven {
        didSet {
            // After this property is set, ensure that we create a a new capture folder
            if ( terrainType != .NONE_SELECTED ) {
    ...
    private var sessionStartInstructions: AVSpeechUtterance? = nil
    ...
    {
        if ( self.terrainType == TerrainType.NONE_SELECTED )
        {
            // BUZZ BUZZ, no type selected
    ...
    func requestNewCaptureFolder() {
        ...
        sessionQueue.async {
            do {
                let newCaptureFolder = try CameraViewModel.createNewCaptureFolder( withTerrainTypePath:self.imageClassString() )
                logger.log("Created new capture folder: \"\(String(describing: newCaptureFolder.captureDir))\"")
                DispatchQueue.main.async {
                    logger.info("Publishing new capture folder: \"\(String(describing: newCaptureFolder.captureDir))\"")
    ...
    func startSetup() {
        do {
            captureFolderState = try CameraViewModel.createNewCaptureFolder( withTerrainTypePath: self.imageClassString() )
        } catch {
    ...
    func imageClassString() -> String? {
        switch(self.terrainType) {
            case TerrainType.NONE_SELECTED:
                return nil
            case TerrainType.bankedLeft:
                return "banked-left"
            case TerrainType.bankedRight:
                return "banked-right"
            case TerrainType.flatEven:
                return "flat-even"
            case TerrainType.flatIrregular:
                return "flat-irregular"
            case TerrainType.grass:
                return "grass"
            case TerrainType.slopedDown:
                return "sloped-down"
            case TerrainType.slopedUp:
                return "sloped-up"
    ...
    private var captureNumber: UInt32 = 0

    // iPhone 14 Plus will barf if we priortize for quality here.
    ...
    private func capturePhotoAndMetadata() {
        ...
        sessionQueue.async {
            ...
            photoSettings.photoQualityPrioritization = .balanced; // self.photoQualityPrioritizationMode

            // Request that the camera embed a depth map into the HEIC output file.
            photoSettings.embedsDepthDataInPhoto = true

            // We likely want this false so that interpolations are not done.
            ...
            let photoCaptureProcessor = self.makeNewPhotoCaptureProcessor(uuid: uuid.uuidString,
                                                                          captureNumber: self.captureNumber,
    ...
    private func makeNewPhotoCaptureProcessor(uuid: String, captureNumber: UInt32, photoSettings: AVCapturePhotoSettings) -> PhotoCaptureProcessor {
    ...
    private func startAutomaticCapture() {
        ...
        // Create an utterance.
//        if ( sessionStartInstructions == nil )
//        {
//            sessionStartInstructions = AVSpeechUtterance(string: "Starting data capture.")
//
...
//                }
//            }
//            sessionStartInstructions?.voice = voiceToUse
//            sessionStartInstructions?.volume = 0.8
////            sessionStartInstructions?.rate = 0.57
//            sessionStartInstructions?.pitchMultiplier = 0.8
////            sessionStartInstructions?.postUtteranceDelay = 0.2
//        }
//
//
...
//        }
//
//        if ( speechSynthesizer != nil && sessionStartInstructions != nil )
//        {
//            // The timer will begin when the instructions are finished speaking
//           speechSynthesizer?.speak(sessionStartInstructions!)
//        }
//        else
//        {
//            // Just start capture
...
//        }
    ...
    {
        if ( utterance == sessionStartInstructions )
        {
            triggerEveryTimer!.start()
            isAutoCaptureActive = true
        }
    ...
    private func stopAutomaticCapture() {
        ...
        terrainType = TerrainType.NONE_SELECTED
    }

    private static func createNewCaptureFolder( withTerrainTypePath imageClassPath:String? ) throws -> CaptureFolderState {
        guard let newCaptureDir = CaptureFolderState.createCaptureDirectory( withTerrainTypePath:imageClassPath  ) else {
    ...
    private func getVideoDeviceForPhotogrammetry() throws -> AVCaptureDevice {
        ...
        if let lidarDepthCameraDevice = AVCaptureDevice.default(.builtInLiDARDepthCamera, for: .video,
                                                                position: .back) {
            logger.log(">>> Got back LIDAR camera!")
            defaultVideoDevice = lidarDepthCameraDevice
        } else if let trueDepthCameraDevice = AVCaptureDevice.default(.builtInTrueDepthCamera, for: .video,
                                                                        position: .back) {
            logger.log(">>> Got back TRUE DEPTH camera!")
...
struct Previews_CameraViewModel_Previews: PreviewProvider {
