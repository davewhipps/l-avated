...
class CaptureFolderState: ObservableObject {
            ...
            CaptureFolderState.workQueue.async {
                ...
                do {
                    ...
                    for imgUrl in imgUrls {
                        ...
                        guard let captureUUID = try? CaptureInfo.extractUUID(from: photoIdString) else {
                            logger.error("Can't get id from from photoIdString: \"\(photoIdString)\"")
                            continue
                        }
                        guard let captureNum = try? CaptureInfo.extractCaptureNumber(from: photoIdString) else {
                            logger.error("Can't get id from from photoIdString: \"\(photoIdString)\"")
                            continue
                        }
                        captureInfoResults.append(CaptureInfo(id: captureUUID, captureNumber: captureNum, captureDir: self.captureDir!))
                    }
    ...
    static func createCaptureDirectory(withTerrainTypePath terrainType:String? ) -> URL? {
        ...
        guard let imageClassPath = terrainType else
        {
            logger.error("Can't create a capture directory with no image class")
            return nil
        }
                                   
        let imageClassFolder = capturesFolder.appendingPathComponent(imageClassPath + "/", isDirectory: true)
        ...
        // Combination of the currently selected class, and a date
        let newCaptureDir = imageClassFolder.appendingPathComponent(timestamp + "/", isDirectory: true)
