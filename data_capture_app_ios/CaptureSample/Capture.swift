...
struct Capture: Identifiable {
    ...
    init(id: String, captureNumber: UInt32, photo: AVCapturePhoto, depthData: Data? = nil, gravity: CMAcceleration? = nil) {
        self.id = id
    ...
    private func writeImage(to captureDir: URL) -> Bool {
        let imageUrl = CaptureInfo.imageUrl(in: captureDir, captureNumber: captureNumber, uuid: id)
    ...
    private func writeGravityIfAvailable(to captureDir: URL) -> Bool {
        ...
        let gravityUrl = CaptureInfo.gravityUrl(in: captureDir, captureNumber: captureNumber, uuid: id)
    ...
    private func writeDepthIfAvailable(to captureDir: URL) -> Bool {
        ...
        let depthMapUrl = CaptureInfo.depthUrl(in: captureDir, captureNumber: captureNumber, uuid: id)
