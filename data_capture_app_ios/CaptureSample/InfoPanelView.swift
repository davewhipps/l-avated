...
/// This is a label that shows the current selected terrain type
struct TerrainTypeLabel: View {
    @ObservedObject var model: CameraViewModel
    
    var body: some View {
        VStack {
            Image(systemName: "mountain.2").foregroundColor(Color.green)
            Menu( model.terrainType.description ) {
                ForEach(CameraViewModel.TerrainType.allCases) { terrain in
                    Button(action: {
                        model.terrainType = terrain
                    })
                    {
                        Label( terrain.description, systemImage: terrain == model.terrainType ? "checkmark" : "" )
                    }
                }
            }
        }
